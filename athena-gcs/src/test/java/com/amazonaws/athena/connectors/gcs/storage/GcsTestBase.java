...
@PrepareForTest({AbstractStorageMetadata.class, StorageDatasourceFactory.class, Job.class, ByteBuffer.class})
public class GcsTestBase extends StorageMock
{
    ...
    {
        PageImpl<Bucket> blob1 = mock(PageImpl.class);
        Storage storage = mock(Storage.class);
        Bucket bucket = mock(Bucket.class);
        PowerMockito.when(storage.list()).thenReturn(blob1);
        PowerMockito.when(storage.list(anyString(), Mockito.any())).thenReturn(tables);
        doReturn(blob1).when(storage).list(anyString());
        PowerMockito.when(blob1.iterateAll()).thenReturn(List.of(bucket));
        PowerMockito.when(bucket.getName()).thenReturn(bucketName);

        return storage;
    }

    protected StorageMetadata getTestDataSource(final String extension) throws FileNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException
    {
...
////        PowerMockito.when(FileCacheFactory.createOnlineGcsStream(storage, bucketName, fileName)).thenReturn(gcsOnlineStream);
        ...
        return new StorageWithStreamTest(storage);
    }
//
...
//
    protected static class StorageWithStreamTest
    {
        ...
        public StorageWithStreamTest(Storage storage)
        {
            requireNonNull(storage, "Storage was null");
            this.storage = storage;
        }
